...
original_name="flask_temp"
original_urlname="flask_temp"
original_description="Awesome flask_temp created by RobinCHoffpauir" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_temp "${name}"
...
then
    ...
    echo "then run 'flask_temp' or 'python -m flask_temp'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_temp' or 'python -m flask_temp' or 'poetry run flask_temp'"
fi
