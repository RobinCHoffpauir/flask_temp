...
- 🎯 Entry points to execute your program using `python -m <flask_temp>` or `$ flask_temp` with basic CLI argument parsing.
...
---
# flask_temp Flask Application

Awesome flask_temp created by RobinCHoffpauir
...
```bash
git clone https://github.com/RobinCHoffpauir/flask_temp flask_temp
cd flask_temp
make install
```
...
```bash
flask_temp create-db   # run once
flask_temp populate-db  # run once (optional)
flask_temp add-user -u admin -p 1234  # ads a user
flask_temp run
```
